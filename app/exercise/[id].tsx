...
import { View, Text, ScrollView, Image, Dimensions, ImageSourcePropType } from 'react-native';
import { useLocalSearchParams } from 'expo-router';
import { exercises } from '../../constants/exercises';
import { Target, Dumbbell, Activity, BarChart2 } from 'lucide-react-native';
        ...
        <View className="bg-white rounded-2xl p-4 mb-6">
          <View className="flex-row items-center justify-between mb-4">
            <View className="flex-row items-center">
              ...
              <Text className="text-gray-900 font-medium capitalize">{exercise.level}</Text>
            </View>
            {exercise.equipment && (
              <View className="flex-row items-center">
                <Dumbbell size={20} className="text-gray-500 mr-2" />
                <Text className="text-gray-900 font-medium capitalize">{exercise.equipment}</Text>
              </View>
            )}
            <View className="flex-row items-center">
              <BarChart2 size={20} className="text-gray-500 mr-2" />
              <Text className="text-gray-900 font-medium capitalize">{exercise.category}</Text>
            </View>
          </View>
            ...
            <View className="flex-row flex-wrap">
              {exercise.primaryMuscles.map((muscle) => (
                <View key={muscle} className="bg-ut_orange bg-opacity-10 rounded-full px-3 py-1 mr-2 mb-2">
                  <Text className="text-ut_orange capitalize">{muscle}</Text>
                </View>
              ))}
